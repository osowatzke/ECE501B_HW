...
\begin{document}
	...
	% v = Each weighted eigenvector component of the input vector is a member of an invariant subspace. As such, each of these components is simply scaled by the application of T. Specifically, each weighted eigenvector is scaled by its respective eigenvector. is  applying T only scales these components. eigenvector is  $\mathbb{R}^5$, and as such, it is mapped back into that subspace. After multiple applications of T, v converges to the eigenvector corresponding to the largest eigenvalue. The eigenvector components corresponding to the smaller eigenvalues are exponentially reduced in size by each application of T
