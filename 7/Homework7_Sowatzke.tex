...
two." Note that the numbers in any column of the matrix must sum to 1, since these numbers correspond to the sum of all possible transition probabilities from a given state.
		...
		\begin{enumerate}
			...
			$\therefore$, the state vector $v_1$ is properly normalized. Note, by the same proof, the state vector at the next time step will also be properly normalized. By induction, we can conclude that the state vector at any time step will be properly normalized.
			...
			$\therefore v_1$ will correspond to the final state vector. Note that we must normalize $v_1$ for it to be a valid state vector.
			...
			The solution of the previous section was an eigenvector, which does not change direction upon application of $T$. In other words, the solution is an element of an invariant subspace. Because the eigenvalue corresponding to $v_{\infty}$ is 1, the result will remain fixed at a final value.
		\end{enumerate}
